...
/** @defgroup MY_SD_Private_Defines SD Private Defines
  * @{
  */
...
/** @defgroup MY_SD_Private_Variables SD Private Variables
  * @{
  */
static uint32_t CardType = MY_CARD_SDSC;

/**
  * @}
  */

/** @defgroup MY_SD_Private_Function_Prototypes SD Private Function Prototypes
  * @{
  */
...
/** @addtogroup MY_SD_Exported_Functions
  * @{
  */
/**
...
/**
  ...
  * @retval SD status
  */
int32_t MY_SD_DeInit(uint32_t Instance)
{
  int32_t ret = BSP_ERROR_NONE;
...
/**
  ...
  */
int32_t MY_SD_DetectITConfig(uint32_t Instance)
{
 ...
 * @retval BSP error status
 */
int32_t MY_SD_IsDetected(uint32_t Instance)
{
  return BSP_ERROR_FEATURE_NOT_SUPPORTED;
}
...
/**
  ...
  * @retval BSP status
  */
int32_t MY_SD_ReadBlocks_DMA(uint32_t Instance, uint32_t *pData, uint32_t BlockIdx, uint32_t BlocksNbr)
{
  int32_t ret;
...
/**
  ...
  * @retval BSP status
  */
int32_t MY_SD_WriteBlocks_DMA(uint32_t Instance, uint32_t *pData, uint32_t BlockIdx, uint32_t BlocksNbr)
{
  int32_t ret;
...
/**
  ...
  * @retval SD status
  */
int32_t MY_SD_ReadBlocks_IT(uint32_t Instance, uint32_t *pData, uint32_t BlockIdx, uint32_t BlocksNbr)
{
  int32_t ret;
...
/**
  ...
  * @retval SD status
  */
int32_t MY_SD_WriteBlocks_IT(uint32_t Instance, uint32_t *pData, uint32_t BlockIdx, uint32_t BlocksNbr)
{
  int32_t ret;
...
/**
  ...
  * @retval SD status
  */
int32_t MY_SD_Erase(uint32_t Instance, uint32_t BlockIdx, uint32_t BlocksNbr)
{
  ...
  else
  {
    ...
    response = SD_SendCmd(SD_CMD_SD_ERASE_GRP_START, BlockIdx * (CardType == MY_CARD_SDHC ? 1U : MY_SD_BLOCK_SIZE), 0xFFU, (uint8_t)SD_ANSWER_R1_EXPECTED);
    SD_IO_CSState(1);
    tmp = SD_DUMMY_BYTE;
    if(BUS_SPIx_Send(&tmp, 1U) != BSP_ERROR_NONE)
    {
    ...
    else
    {
      if ((uint8_t)(response & 0xFFU) == (uint8_t)SD_R1_NO_ERROR)
      {
        ...
        response = SD_SendCmd(SD_CMD_SD_ERASE_GRP_END, ((BlockIdx + BlocksNbr)*MY_SD_BLOCK_SIZE) * (CardType == MY_CARD_SDHC ? 1U : MY_SD_BLOCK_SIZE), 0xFFU, (uint8_t)SD_ANSWER_R1_EXPECTED);
        SD_IO_CSState(1);
        if(BUS_SPIx_Send(&tmp, 1U) != BSP_ERROR_NONE)
        {
          ret = BSP_ERROR_PERIPH_FAILURE;
...
/** @defgroup MY_SD_Private_Functions SD Private Functions
  * @{
  */
/**
...
static void SD_IO_DeInit(void)
{
  ...
  /* Configure SD_CS_PIN pin: SD Card CS pin */
  GPIO_InitStruct.Pin = MY_SD_CS_PIN;
  HAL_GPIO_DeInit(MY_SD_CS_GPIO_PORT, GPIO_InitStruct.Pin);

  /* SD_CS_GPIO Periph clock enable */
  MY_SD_CS_GPIO_CLK_DISABLE();
}
